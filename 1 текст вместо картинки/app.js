...
buttons.forEach((btn) => btn.addEventListener('click', (event) => handleClick(event)));
...
function showMoreText(card) {
    ...
    // 6.1 сохраняем произвольный текст в переменную
	 let txt = 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolorem esse distinctio fugit aut quae commodi cupiditate! Eos, perferendis iste illo architecto facilis, eaque impedit id dolor nulla unde iusto! Quas assumenda voluptas blanditiis minima officiis necessitatibus accusantium reprehenderit explicabo ab facere nihil, placeat similique repellendus nesciunt voluptatibus aliquid? Aperiam, saepe.'	
    
    // 6.2 внутри .product есть .productName, после него вставляем div.desc и текстом из переменной из п. 6.1
	 card.productName.insertAdjacentHTML('afterbegin', `<div class='desc'>${txt}</div>`);
